...
export const applyForRider = async (req, res, next) => {
    try {
        ...
        const existingApp = await RiderApplication.findOne({ userEmail: req.user.email });
...
// Get current user's rider application status (FAST DIRECT LOOKUP)
export const getMyRiderApplication = async (req, res, next) => {
    try {
        ...
        // Direct 1-step lookup using userEmail indexed in token
        const application = await RiderApplication.findOne({ userEmail: req.user.email }).lean();
...
export const updateDeliveryStatus = async (req, res, next) => {
    try {
        ...
        const { status, message, otp } = req.body;
